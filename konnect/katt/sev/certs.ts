// --- AMD Genoa (Zen4) ---

/** ARK-Genoa: AMD Root Key, CN=ARK-Genoa, RSA-4096, self-signed RSA-PSS SHA-384, valid 2022-2047 */
export const ARK_GENOA_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

/** ASK-Genoa: AMD SEV Key, CN=SEV-Genoa, RSA-4096, signed by ARK-Genoa, valid 2022-2047 */
export const ASK_GENOA_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

// --- AMD Milan (Zen3) ---

/** ARK-Milan: AMD Root Key, CN=ARK-Milan, RSA-4096, self-signed RSA-PSS SHA-384, valid 2020-2045 */
export const ARK_MILAN_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

/** ASK-Milan: AMD SEV Key, CN=SEV-Milan, RSA-4096, signed by ARK-Milan, valid 2020-2045 */
export const ASK_MILAN_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

// --- AMD Turin (Zen5) ---

/** ARK-Turin: AMD Root Key, CN=ARK-Turin, RSA-4096, self-signed RSA-PSS SHA-384, valid 2023-2048 */
export const ARK_TURIN_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

/** ASK-Turin: AMD SEV Key, CN=SEV-Turin, RSA-4096, signed by ARK-Turin, valid 2023-2048 */
export const ASK_TURIN_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

// --- Product detection ---

import { extractIssuerCN, parseCertificate } from "../der.ts";

export type SevProduct = "Milan" | "Genoa" | "Turin";

/**
 * Detect AMD product family from VCEK certificate by parsing the issuer CN.
 * AMD VCEK certs have issuer CN like "SEV-Milan", "SEV-Genoa", or "SEV-Turin".
 */
export function detectProductFromVcek(vcekDer: Uint8Array): SevProduct {
  const cert = parseCertificate(vcekDer);
  const issuerCN = extractIssuerCN(cert.tbs);

  if (issuerCN.includes("Milan")) return "Milan";
  if (issuerCN.includes("Genoa")) return "Genoa";
  if (issuerCN.includes("Turin")) return "Turin";
  throw new Error(
    `SEV-SNP: cannot determine product from VCEK issuer CN "${issuerCN}" (expected Milan, Genoa, or Turin)`,
  );
}

/** Get the ARK and ASK PEM strings for a given product */
export function getCertsForProduct(product: SevProduct): { arkPem: string; askPem: string } {
  if (product === "Milan") return { arkPem: ARK_MILAN_PEM, askPem: ASK_MILAN_PEM };
  if (product === "Turin") return { arkPem: ARK_TURIN_PEM, askPem: ASK_TURIN_PEM };
  return { arkPem: ARK_GENOA_PEM, askPem: ASK_GENOA_PEM };
}
